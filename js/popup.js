...
async function saveOpenSupportTicketNumber() {
  return await runInOpenTab(() => {
    let ticketNumber = jQuery.trim(jQuery('.header-primary__page-name .breadcrumb__item.active').text());
...
function loadReport() {
  ...
  chrome.storage.local.get('report', function (result) {
    ...
    if (result && result.report) {
      // console.log(prettyPrintJson.toHtml(JSON.parse(result.report)))
      jQuery('#report-data').html(prettyPrintJson.toHtml(JSON.parse(result.report)));
    } else {
